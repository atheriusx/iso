...
class SEHIRA extends Client {
      constructor (...options) {
        ...
        async fetchCommands(active = true, slash = true) {
            if(slash) {
            ...
            if(!active) return;
            let dirs = fs.readdirSync("./Server/Distributors/_commands", { encoding: "utf8" });
        ...
        async fetchEvents(active = true) {
            if(!active) return;
            let dirs = fs.readdirSync('./Server/Distributors/_events', { encoding: "utf8" });
            dirs.forEach(dir => {
        ...
        connect(token) {
            ...
            this.login(token).then(sehira => {
                ...
                this.on("ready", async () => {
                    ...
                    let kanal = this.channels.cache.get("1012834430588559360")
